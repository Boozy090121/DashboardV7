...
// Mock data (since we don't have access to the actual Excel files)
const mockData = {
  overview: {
    totalRecords: 1245,
    totalLots: 78,
...
const NovoNordiskDashboard = () => {
  ...
  const ProcessTimelineChart = () => {
    return (
      <div className="bg-white rounded-lg shadow p-4 h-64">
        ...
        <ResponsiveContainer width="100%" height="80%">
          <LineChart
            data={mockData.overview.processTimeline}
            margin={{ top: 5, right: 30, left: 20, bottom: 5 }}
          >
            <CartesianGrid strokeDasharray="3 3" />
            <XAxis dataKey="month" />
  ...
  const FileStatus = () => {
    const [fileStatuses, setFileStatuses] = useState<Record<string, FileStatusType>>({
    ...
    const handleRefreshData = () => {
      ...
      setFileStatuses(prev => {
        const newStatuses: Record<string, FileStatusType> = {};
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-4">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {Object.entries(fileStatuses).map(([filename, data]) => (
            <div key={filename} className={`px-3 py-1 ${(data as FileStatusType).status === 'error' ? 'bg-red-50' : 'bg-gray-100'} rounded-md text-sm flex items-center`}>
              <FileText size={14} className="mr-1" />
              {filename}
              <span className="ml-2">{getStatusIcon((data as FileStatusType).status)}</span>
            </div>
          ))}
        </div>
      </div>
